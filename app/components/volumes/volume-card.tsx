...
export function VolumeCard({ volume, projectId }: VolumeCardProps) {
  ...
  return (
    <div className="group relative overflow-hidden rounded-lg border border-stone-200 bg-white shadow-sm transition-shadow hover:shadow-sm">
      <Link
      ...
      >
        ...
        {/* Body */}
        <div className="p-3">
          <h3 className="truncate font-serif text-sm font-semibold text-stone-700">
            {volume.name}
          </h3>
          <p className="mt-0.5 font-mono text-xs text-stone-500">{volume.referenceCode}</p>
          <div className="mt-2 flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-1.5">
              {openFlagCount > 0 && (
                <span
                  className="inline-flex items-center gap-1 rounded-full bg-madder-tint px-2 py-0.5 font-sans text-xs font-semibold text-madder-deep"
              ...
              <span
                className={`inline-flex items-center rounded-full px-2 py-0.5 font-sans text-xs font-semibold ${statusBadgeColors[volume.status] || "bg-stone-200 text-stone-500"}`}
              >
      ...
      {canDelete && (
        ...
        >
          ...
          >
            <svg className="h-4 w-4 text-indigo" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
