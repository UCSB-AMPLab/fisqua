export default {
  toolbar: {
    undo: "Undo",
    undo_shortcut: "Undo (Ctrl+Z)",
    redo: "Redo",
    redo_shortcut: "Redo (Ctrl+Shift+Z)",
    add_boundary: "Add boundary",
    delete_boundary: "Delete boundary",
    zoom_in: "Zoom in",
    zoom_out: "Zoom out",
    fit_to_width: "Fit to width",
    go_to_image: "Go to image",
    back_to_volumes: "Back to volumes",
  },
  save_status: {
    saved: "Saved",
    saving: "Saving...",
    unsaved: "Unsaved",
  },
  outline: {
    title: "Outline",
    hint: "Click between images to add boundaries",
    page_boundary: "Page boundary",
    within_page_boundary: "Within-page boundary",
    document: "Document",
    blank: "Blank",
    continuation: "Continuation",
    front_matter: "Front matter",
    back_matter: "Back matter",
    no_title: "Untitled",
    no_type: "(unset)",
    type_label: "Type",
    title_label: "Title",
    ref_label: "Ref.",
    level_label: "Level",
    delete_boundary: "Delete boundary",
    confirm_delete: "Delete?",
    confirm_delete_tooltip: "Confirm deletion",
    indent_tooltip: "Nest under previous item",
    outdent_tooltip: "Move to parent level",
    type: {
      item: "Document",
      blank: "Blank",
      front_matter: "Front matter",
      back_matter: "Back matter",
    },
    comments_label: "Comments",
    cataloguer_note_label: "Cataloguer notes",
    cataloguer_note_placeholder: "Add notes about this entry...",
    reviewer_comment_field_label: "Reviewer comments",
    reviewer_comment_placeholder: "Add comments about this entry...",
    has_note: "Has cataloguer notes",
    has_reviewer_comment: "Has reviewer comments",
    char_counter: "{{count}}/{{max}}",
    reviewer_comment_label: "Reviewer comment:",
    accepting: "Accepting...",
    readonly: {
      segmented: "Submitted for review",
      approved: "This volume has been approved",
      not_assigned: "Read-only -- you are not assigned to this volume",
    },
  },
} as const;
